...
function renderName() {
  ...
  const characterCountEl = fullname.previousElementSibling.querySelector(".count");
...
function renderUsername() {
  ...
  const characterCountEl = username.previousElementSibling.querySelector(".count");
...
function renderMessage() {
  ...
  if (messageValue === "") {
    tweetMessage.innerText = "Generate fake tweet images";
  } else {
...
function renderTime() {
  ...
  var formatetime = new Date(`7/10/2013 ${timeValue}`).toLocaleTimeString().replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3");
  if (timeValue === "") {
    tweetTime.innerText = getCurrentTime();
  } else {
    tweetTime.innerText = formatetime;
  }
}
...
function renderDate() {
  ...
  const formatingdate = new Date(dateValue);
  const year = formatingdate.getFullYear();
  const datea = formatingdate.getDate();
  const monthIndex = formatingdate.getMonth();
  const monthName = MONTHS[monthIndex];

  if (dateValue === "") {
    tweetDate.innerText = getCurrentDate();
  } else {
    tweetDate.innerText = `${monthName} ${datea}, ${year}`;
  }
}
