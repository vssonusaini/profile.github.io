...
const makeLabels = () => {
 ...
 // cart id print
 barcode_preview.innerHTML = barcode_lable(
  `${set_label_value}-${getDB.formInput.cartId}`,
  13
 );
 ...
 for (i = 0; i < getDB.formInput.nmLabels; i++) {
  ...
  barcode_preview.innerHTML += barcode_lable(
   `${set_label_value}${getDB.formInput.cartId}${cart_position}-0${cart_position_no}`,
   i
  );
 }
...
const AddValueOneBarcode = () => {
 ...
 database.oneBarcode.barcodeformte = barcodeformte.value;
 localStorage.setItem(dbname, JSON.stringify(database));
};
...
const MakeSingBarcode = () => {
 ...
 barcodeformte.value = database.oneBarcode.barcodeformte;
...
barcodeformte.addEventListener("change", () => {
 AddValueOneBarcode();
 MakeSingBarcode();
});

barcodevalue.addEventListener("keyup", () => {
 AddValueOneBarcode();
 MakeSingBarcode();
});
